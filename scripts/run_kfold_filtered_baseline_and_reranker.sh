...
#SBATCH --job-name=kfold_filtered_baseline_reranker
#SBATCH --output=kfold_filtered_baseline_reranker_%j.log
...
echo "=========================================="
echo "Starting K-Fold Experiments (Filtered Chunk Pool)"
echo "Date: $(date)"
echo "GPU: $CUDA_VISIBLE_DEVICES"
echo "=========================================="

# Experiment 1: Baseline (filtered pool, no reranker)
echo ""
echo "==================== EXPERIMENT 1: BASELINE (FILTERED) ===================="
echo "Model: BAAI/bge-m3"
echo "Page-k: 20 | Chunk-k: 5 | Reranker: OFF"
echo "========================================================================="
...
python train_k_fold2_filtered_chunks.py \
  ...
  --output-dir "$SCRATCH_OUT/kfold_filtered_baseline_bge_m3_p20" \
  --pdf-dir "pdfs" \
  --seed 42

if [ $? -eq 0 ]; then
	echo "✓ Experiment 1 (Baseline) completed successfully"
else
	echo "✗ Experiment 1 (Baseline) failed"
fi

# Experiment 2: Reranker (filtered pool + cross-encoder)
echo ""
echo "==================== EXPERIMENT 2: RERANKER (FILTERED) ===================="
echo "Model: BAAI/bge-m3"
...
python train_k_fold2_filtered_chunks.py \
  ...
  --output-dir "$SCRATCH_OUT/kfold_filtered_reranker_bge_m3_p20" \
  --pdf-dir "pdfs" \
  --seed 42

if [ $? -eq 0 ]; then
	echo "✓ Experiment 2 (Reranker) completed successfully"
else
	echo "✗ Experiment 2 (Reranker) failed"
fi
