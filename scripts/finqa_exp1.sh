...
#SBATCH --job-name=exp1_hier_loss
#SBATCH --output=logs/exp1_hier_loss_%j.log
#SBATCH --error=logs/exp1_hier_loss_%j.log
...
# =============================================================================
# Experiment 1 — Hierarchical loss only
#
# Ablation flag : --use-hierarchical-loss
# Change vs job-6000 baseline:
#   Loss   : L = 0.7 * MNR(query, page) + 0.3 * MNR(query, chunk)
#   Chunk  : BM25-best 512-tok window within the gold page
#   ChunkNeg: BM25-best chunk from each random page negative
#   LoRA   : unchanged (q/k/v only)
#   Negs   : unchanged (random page sampling)
#
# Batch notes:
#   Standard loss  (5 cols)  : 8 × 5 = 40 texts/step  — same as baseline
#   Hierarchical   (9 cols)  : 4 × 9 = 36 texts/step  — reduced micro-batch
#   Chunk cols are at most 512 tok, so actual GPU memory stays comparable.
#   Effective batch = 4 × 8 = 32 (same as baseline 8 × 4).
# =============================================================================
...
PDF_DIR_FB="pdfs"
MODEL_OUT="models/exp1_hier_loss"
EVAL_OUT="outputs/exp1_hier_loss"
LLM="Qwen/Qwen2.5-7B-Instruct"

# ── Training hyperparameters ───────────────────────────────────────────────
EPOCHS=10
BATCH_SIZE=4        # reduced from 8 — 9 text cols × 4 = 36 texts/step
GRAD_ACCUM=8        # doubled — effective batch = 4×8 = 32 (unchanged)
LR=2e-5
LORA_R=16
LORA_ALPHA=32
HARD_NEGS=3
CROSS_DOC_NEGS=0    # disabled for this ablation (isolates hier-loss effect)
LOSS_SCALE=20.0     # original scale (isolates hier-loss effect)
HIER_ALPHA=0.7      # 0.7 * L_page + 0.3 * L_chunk
CHUNK_TOKENS=512
...
echo "========================================================="
echo "Experiment     : 1 — Hierarchical loss"
...
echo "========================================================="
echo "Ablation flags : --use-hierarchical-loss"
...
# Ensure rank_bm25 is available (needed when --use-bm25-hard-negs is set;
# not strictly required for this experiment but harmless to have installed)
...
# ── Stage 1: Training ───────────────────────────────────────────────────────
echo ""
echo ">>> STAGE 1: LoRA fine-tuning (hierarchical loss)"
echo "    9 sentence columns: query, page_pos, 3×page_neg, chunk_pos, 3×chunk_neg"
echo "========================================================="
