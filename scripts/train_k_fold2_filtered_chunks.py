...
class KFoldConfig:
    ...
    max_seq_length: int = 1536           # Inference seq length: covers 95th+ percentile of page lengths (~6000 chars)
    training_max_seq_length: int = 512   # Training seq length: shorter to fit GPU memory (MNRL signal is preserved)

    # Training settings
    epochs: int = 15
    batch_size: int = 32  # Larger batches = more in-batch negatives for MNRL
    ...
    chunk_size: int = 384   # in tokens; finer granularity – median page (~840 tokens) → ~2-3 chunks
    chunk_overlap: int = 64  # in tokens; ~17% overlap

    # Reranker settings
    use_reranker: bool = False
    reranker_type: str = "cross_encoder"  # cross_encoder, ot, ot_then_cross_encoder
    reranker_model: str = "BAAI/bge-reranker-v2-m3"
    reranker_candidates: int = 50
    reranker_top_k: int = 5
    reranker_batch_size: int = 16
    ot_model: str = "BAAI/bge-m3"
    ot_query_sentences: int = 8
    ot_doc_sentences: int = 24
    ot_reg: float = 0.05
    ot_iters: int = 40
    ot_prune_k: int = 20
...
def train_fold_model(
    ...
    # Initialize model
    # Use a shorter max_seq_length during training to fit GPU memory.
    # BGE-M3 at seq_len=1536 + batch_size=32 fills a 44GB GPU with activations.
    # MNRL training signal is preserved at 512 tokens; full seq_len is restored
    # after training for inference.
    model = SentenceTransformer(config.base_model_name)
    model.max_seq_length = config.training_max_seq_length
    logger.info(f"Training max_seq_length: {config.training_max_seq_length} | Inference max_seq_length: {config.max_seq_length}")
    ...
    # Restore full inference seq length before returning
    model.max_seq_length = config.max_seq_length
    logger.info(f"Restored inference max_seq_length to {config.max_seq_length}")
...
def run_page_then_chunk_inference(
    ...
    # Build global chunk pool with page->chunk mapping (for filtered retrieval)
    logger.info("Building global chunk pool (for page-filtered retrieval)...")
    ...
    for doc_name, pages in pages_by_doc.items():
        for page in pages:
            ...
            for chunk_text in page_chunks:
                ...
                meta = {"doc_name": doc_name, "page": page.page_num}
                all_chunk_metas.append(meta)
                page_to_chunk_indices.setdefault((doc_name, page.page_num), []).append(idx)

    logger.info("Encoding chunks for filtered retrieval with %s...", config.embedding_model)
    chunk_embeddings: List[torch.Tensor] = []
    if all_chunk_texts:
        for i in range(0, len(all_chunk_texts), config.batch_size):
            batch = all_chunk_texts[i : i + config.batch_size]
            emb = chunk_embeddings_model.encode(batch, convert_to_tensor=True)
            chunk_embeddings.append(F.normalize(emb, p=2, dim=1))
    embedding_dim = chunk_embeddings_model.get_sentence_embedding_dimension()
    ...
    logger.info(
        f"Global chunk pool built: {len(all_chunk_texts)} chunks from {len(pages_by_doc)} documents"
    )
    ...
    if config.use_reranker:
        try:
            if config.reranker_type in ["ot", "ot_then_cross_encoder"]:
                ot_reranker = OTReranker(
                    model_name=config.ot_model,
                    device="cuda" if torch.cuda.is_available() else "cpu",
                    query_max_sentences=config.ot_query_sentences,
                    doc_max_sentences=config.ot_doc_sentences,
                    sinkhorn_reg=config.ot_reg,
                    sinkhorn_iters=config.ot_iters,
                )
                logger.info(f"Loaded OT reranker: {config.ot_model}")
            if config.reranker_type in ["cross_encoder", "ot_then_cross_encoder"]:
    ...
    for sample in tqdm(samples, desc="Inference"):
        ...
        # Step 2: Retrieve top-P pages across ALL documents (learned page scorer)
        ...
        # When using reranker, retrieve MORE pages (top-20) so reranker has more chunks to choose from
        # DON'T rerank pages - the learned model was trained for this and knows best
        if config.use_reranker and reranker:
            # Retrieve top-20 pages (gives reranker more diverse chunks)
            pages_to_retrieve = min(20, len(all_pages))
            top_page_indices = torch.argsort(scores, descending=True)[:pages_to_retrieve].cpu().tolist()
            logger.debug(f"Retrieved top-{pages_to_retrieve} pages for chunk reranking")
        else:
            # Standard retrieval: top-k pages only
        ...
        # Step 3: Retrieve chunks ONLY from top-P pages, then rank within that subset (BGE-M3)
        retrieved_chunks = []
        candidate_indices: List[int] = []
        for doc, page in retrieved_pages_meta:
            candidate_indices.extend(page_to_chunk_indices.get((doc, page), []))

        if candidate_indices and chunk_embeddings_tensor.numel() > 0:
            candidate_indices = list(dict.fromkeys(candidate_indices))
            candidate_embs = chunk_embeddings_tensor[candidate_indices]
            query_emb = chunk_embeddings_model.encode(question, convert_to_tensor=True)
            query_emb = F.normalize(query_emb, p=2, dim=0)
            scores = torch.matmul(candidate_embs, query_emb)

            if config.use_reranker:
                candidate_k = max(config.reranker_candidates, config.chunk_k * 5)
            else:
                candidate_k = config.chunk_k

            candidate_k = min(candidate_k, scores.numel())
            if candidate_k > 0:
                top_local = torch.topk(scores, k=candidate_k).indices.tolist()
                for local_idx in top_local:
                    global_idx = candidate_indices[local_idx]
                    retrieved_chunks.append(
                        {
                            "text": all_chunk_texts[global_idx],
                            "metadata": all_chunk_metas[global_idx],
        ...
        if reranker and retrieved_chunks:
            try:
                if config.reranker_type == "ot":
                    doc_texts = [c["text"] for c in retrieved_chunks]
                    scores = ot_reranker.score(question, doc_texts)
                elif config.reranker_type == "ot_then_cross_encoder":
                    doc_texts = [c["text"] for c in retrieved_chunks]
                    ot_scores = ot_reranker.score(question, doc_texts)
                    ot_ranked = sorted(
                        zip(retrieved_chunks, ot_scores),
                        key=lambda x: x[1],
                        reverse=True
                    )
                    prune_k = min(max(1, config.ot_prune_k), len(ot_ranked))
                    pruned_chunks = [chunk for chunk, _ in ot_ranked[:prune_k]]
                    rerank_pairs = [(question, c["text"]) for c in pruned_chunks]
                    ce_scores = ce_reranker.predict(rerank_pairs, batch_size=config.reranker_batch_size)
                    ranked = sorted(
                        zip(pruned_chunks, ce_scores),
                        key=lambda x: x[1],
                        reverse=True
                    )
                    top_k = config.reranker_top_k or config.chunk_k
                    retrieved_chunks = [
                    ...
                    rerank_pairs = [(question, c["text"]) for c in retrieved_chunks]
                    scores = ce_reranker.predict(rerank_pairs, batch_size=config.reranker_batch_size)
                ...
                logger.warning(f"Reranking failed for question: {e}")
...
def run_kfold_cross_validation(config: KFoldConfig) -> Dict[str, Any]:
    ...
    # Chunk retriever model (BGE-M3) is separate from the learned page scorer
    chunk_embeddings_model = SentenceTransformer(config.embedding_model)
    logger.info("Chunk retriever model: %s", config.embedding_model)
    ...
    for fold_idx in range(config.n_folds):
        ...
        fold_predictions = run_page_then_chunk_inference(
            ...
            config=config,
            chunk_embeddings_model=chunk_embeddings_model  # Use base model for chunks
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base-model", type=str, default="BAAI/bge-m3", help="Base embedding model")
    ...
    parser.add_argument("--reranker-type", type=str, default="cross_encoder", choices=["cross_encoder", "ot", "ot_then_cross_encoder"], help="Reranker type")
    parser.add_argument("--reranker-model", type=str, default="BAAI/bge-reranker-v2-m3", help="Reranker model name")
    parser.add_argument("--reranker-candidates", type=int, default=50, help="Candidates to rerank")
    parser.add_argument("--reranker-top-k", type=int, default=5, help="Top K after reranking")
    parser.add_argument("--reranker-batch-size", type=int, default=16, help="Reranker batch size")
    parser.add_argument("--ot-model", type=str, default="BAAI/bge-m3", help="Embedding model for OT reranker")
    parser.add_argument("--ot-query-sentences", type=int, default=8, help="Max query sentences for OT")
    parser.add_argument("--ot-doc-sentences", type=int, default=24, help="Max document sentences for OT")
    parser.add_argument("--ot-reg", type=float, default=0.05, help="Sinkhorn regularization for OT")
    parser.add_argument("--ot-iters", type=int, default=40, help="Sinkhorn iterations for OT")
    parser.add_argument("--ot-prune-k", type=int, default=20, help="Top-K to keep after OT before CE in ot_then_cross_encoder")
    ...
    config = KFoldConfig(
        ...
        ot_model=args.ot_model,
        ot_query_sentences=args.ot_query_sentences,
        ot_doc_sentences=args.ot_doc_sentences,
        ot_reg=args.ot_reg,
        ot_iters=args.ot_iters,
        ot_prune_k=args.ot_prune_k,
