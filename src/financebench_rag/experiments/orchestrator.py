...
from ..data import FinanceBenchLoader
...
from .modes import (
    run_closed_book as _run_closed_book,
    run_open_book as _run_open_book,
    run_random_single_store as _run_random_single_store,
    run_shared_vector as _run_shared_vector,
    run_single_vector as _run_single_vector,
)
...
def setup_logging(experiment_name: str, log_dir: Optional[str] = None):
    ...
    if log_dir is None:
        log_dir = str((Path.cwd() / "logs").resolve())
...
class RAGExperiment(
    ...
    def __init__(self, 
        ...
        if output_dir is None:
            output_dir = str((working_dir / "outputs").resolve())
        if vector_store_dir is None:
            vector_store_dir = str((working_dir / "vector_stores").resolve())
        self.output_dir = str(Path(output_dir).resolve())
        self.vector_store_dir = str(Path(vector_store_dir).resolve())
        # Local PDF directory defaults to ./pdfs relative to the working directory.
        if pdf_local_dir is None:
            self.pdf_local_dir = (working_dir / "pdfs").resolve()
        else:
            self.pdf_local_dir = Path(pdf_local_dir).resolve()
...
def main():
    ...
    working_dir = Path.cwd()
    default_pdf_dir = str((working_dir / "pdfs").resolve())
    default_vector_store_dir = str((working_dir / "vector_stores").resolve())
    default_output_dir = str((working_dir / "outputs").resolve())
