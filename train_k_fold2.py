...
def run_page_then_chunk_inference(
    ...
    # Use embedding_model tokenizer for chunking (same model used to embed chunks)
    ...
    for sample in tqdm(samples, desc="Inference"):
        ...
        # Get candidate chunks (retrieve MANY more when reranking to see improvement)
        if config.use_reranker:
            # For reranking: get chunks from top-20 pages, retrieve many candidates
            # With 20 pages, we'll have ~100-300 chunks depending on page lengths
            candidate_k = max(config.reranker_candidates, 200)
        else:
            # Standard: fewer candidates needed
            candidate_k = config.chunk_k * 5
        ...
        try:
            ...
            for text, metadata, score in candidates:
                ...
                if (chunk_doc, chunk_page) in retrieved_page_set:
                    ...
                    # Don't stop early if reranking - need all candidates
                    if not config.use_reranker and len(retrieved_chunks) >= config.chunk_k:
                        break
